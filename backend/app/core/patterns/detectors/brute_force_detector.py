...
class BruteForceDetector(BasePatternDetector):
    """Detector de algoritmos de fuerza bruta MEJORADO"""
    ...
    def detect(self, ast: ASTNode) -> PatternMatch:
        ...
        # 2. Verificar patrón de swap (MUY IMPORTANTE)
        swap_pattern = self._indicators[1]
        if analysis["has_swap_pattern"]:
            swap_pattern.found = True
            swap_pattern.evidence = "Patrón de intercambio detectado (típico de ordenamiento fuerza bruta)"
            indicators_found.append(swap_pattern)
        else:
            indicators_missing.append(swap_pattern)

        # 3. Verificar búsqueda exhaustiva
        ...
        # 4. Verificar ausencia de poda
        no_pruning = self._indicators[3]
        pruning_ratio = analysis["conditionals"] / max(analysis["loops"], 1)
        if pruning_ratio < 0.5:
            no_pruning.found = True
            no_pruning.evidence = f"Pocas condiciones de poda (ratio: {pruning_ratio:.2f})"
            indicators_found.append(no_pruning)
        else:
            indicators_missing.append(no_pruning)

        # 5. Verificar ausencia de memoización
        no_memo = self._indicators[4]
    ...
    def _build_reasoning(
        ...
        if analysis["max_loop_depth"] >= 2:
            reasons.append(
                f"loops anidados (profundidad {analysis['max_loop_depth']})"
            )
        ...
        return (
            f"El algoritmo usa fuerza bruta: {', '.join(reasons)}. "
